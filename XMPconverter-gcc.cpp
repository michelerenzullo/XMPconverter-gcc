...
void encode(string path, string& outFileName) {
	...
	for (int32 idx = 0, readed; idx < input_size * input_size * input_size * 3; ++s)
	 if (*s == '\n' && (sscanf(s, "%lf %lf %lf%n", &samples_1[idx], &samples_1[idx + 1], &samples_1[idx + 2], &readed)) == 3) {
	  idx += 3 ; s += readed-1;
	 }
	 delete text;*/
	char* points = nullptr;
...
int main(int argc, char** argv) {
	...
	for (int32 option_index = 0, c = getopt_long(argc, argv, "t:s:m:x:a:p:o:g:T:G:h?", long_options, &option_index); c != -1; c = getopt_long(argc, argv, "t:s:m:x:a:p:o:g:T:G:h?", long_options, &option_index)) {
		...
		case 't':
			if (strcmp(optarg, "medium") == 0) options.strength = "   crs:ToneMapStrength=\"1\"\n";
			else if (strcmp(optarg, "high") == 0) options.strength = "   crs:ToneMapStrength=\"2\"\n";
			break;
